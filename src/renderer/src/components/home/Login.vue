...
const emits = defineEmits(['switch-component', 'set-csrfmiddlewaretoken', 'set-user', 'set-perms'])
const props = defineProps(['user', 'csrfmiddlewaretoken'])
...
function clickLogin() {
  ...
  instance.proxy.$axios.post(form.getAttribute("action"), form_data).then(response => {
    ...
    emits('set-csrfmiddlewaretoken', data['csrfmiddlewaretoken'])
